...
# Slow integration tests that verify TLS encryption at the wire level.
# Run separately: bundle exec ruby test/tls_encryption_test.rb
...
def make_tlse_server_cert(ca_cert, ca_key, cn: '127.0.0.1')
  key = OpenSSL::PKey::RSA.new(2048)
  cert = OpenSSL::X509::Certificate.new
  cert.version = 2
  cert.serial = 2
...
TLSE_CA_CERT, TLSE_CA_KEY = make_tlse_ca
TLSE_SERVER_CERT, TLSE_SERVER_KEY = make_tlse_server_cert(TLSE_CA_CERT, TLSE_CA_KEY)
...
describe 'TLS encryption analysis' do
  describe 'tcpdump' do
    before do
    ...
    it 'plaintext is visible in TCP capture but not in TLS capture' do
      ...
      Async do |task|
        rep = NNG::Socket::Rep0.new
        rep.listen(tls_url, cert: TLSE_SERVER_CERT, key: TLSE_SERVER_KEY)
  ...
  describe 'raw TCP negative test' do
    it 'raw TCP socket cannot read TLS plaintext' do
      ...
      Async do |task|
        rep = NNG::Socket::Rep0.new
        rep.listen(url, cert: TLSE_SERVER_CERT, key: TLSE_SERVER_KEY)
